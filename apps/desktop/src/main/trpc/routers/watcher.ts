...
import { setWatcherScopeInputSchema } from "@shared/contracts/watcher/watcher.contract";
import { getDatabase } from "../../db";
import { vaultWatcherManager } from "../../vault-watcher-manager";
import { publicProcedure, router } from "../trpc";

export const watcherRouter = router({
  setScope: publicProcedure.input(setWatcherScopeInputSchema).mutation(({ input }) => {
    if (input.type === "idle") {
