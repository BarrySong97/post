...
- `apps/desktop/src/renderer/src/components/layout/file-drop-zone.tsx` - global drop mask and import trigger on the app shell.
...
- Local drag-and-drop import: dropping external files onto the app shell (when a vault is active) shows an inhale-style hover mask, then `assets.importLocalFiles` copies into `assets/imports/` (folders keep relative structure under `assets/imports/<folderName>/`). Hidden path segments and sources already inside the vault are skipped. After copy, main runs indexer `refresh` on the new relative paths and queues thumbnails; the board invalidates through the shared vault refresh path. Paths are resolved in preload via `webUtils.getPathForFile` (`window.api.resolveDroppedFilePaths`).
...
- Video cards show a top-right duration badge when `imageCache.videoDurationMs` is present (filled by the indexer via ffprobe during thumbnail generation).
