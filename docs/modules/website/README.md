...
`apps/website` is the standalone web surface for Post, built with Next.js (App Router), React 19, Tailwind CSS v4, and HeroUI (`@heroui/react` v3). It hosts the public/marketing landing page, changelog, and statically generated MDX blog, separate from the Electron [desktop](../desktop/README.md) app. The landing page reproduces a reference layout (nav, hero, interactive product preview, how-it-works, FAQ, footer). Text uses the self-hosted Geist font (`geist` package, applied in `layout.tsx`). The nav shows the app icon (`public/post-icon.png`, copied from the desktop icons) plus the name "Post"; product preview data comes from [mock-data](../mock-data/README.md), while the preview UI comes from [ui](../ui/README.md).
...
- `apps/website/app/blog/page.tsx` and `app/blog/[slug]/page.tsx` - static blog index and article routes generated from the Velite collection.
...
- `apps/website/content/blog/` - source MDX articles and frontmatter consumed at build time.
- `apps/website/velite.config.ts` - blog collection schema, MDX compilation, heading IDs, TOC, and generated post metadata.
- `apps/website/scripts/img.mjs` and `scripts/lib/` - optional article-image compression, ThumbHash generation, R2 upload, and MDX rewrite pipeline.
...
- Velite compiles `content/blog/*.mdx` before Next dev/build and writes the ignored `.velite/` module imported by blog routes. Published posts also feed individual entries into `sitemap.xml`.
- `pnpm -F website img` can normalize local or remote article images to WebP, generate ThumbHash metadata, upload to the configured R2 bucket, and replace Markdown image syntax with the shared `BlogImage` component. Secrets stay in the ignored website `.env`; `.env.example` documents the required names.
...
- Before running `pnpm release <version>`, add a dated, user-facing entry at the start of `ReleaseTimeline`, move the single `badge: "latest"` marker to it, and keep its version in sync with the desktop and website package versions; the release helper validates this ordering before it mutates git state.
