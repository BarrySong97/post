...
- `packages/ui/src/styles/styles.css` - desktop-only Electron window chrome and status-line styles. Imports `./theme.css` and adds window-drag rules, panel-animation, and the global status line CSS.
...
- Keep `theme.css` free of window-chrome assumptions (no `overflow: hidden`, transparent body background, or minimum window width) since it's consumed outside Electron too.
