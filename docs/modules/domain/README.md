...
- Keep saved-view filter serialization compatible with desktop renderer contracts, including the first-class `post` and `youtube` asset types used by browser captures and `added_desc` as the fallback sort for missing or invalid saved-view sort JSON.
